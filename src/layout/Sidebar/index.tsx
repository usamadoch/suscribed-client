...
const Sidebar = ({ }: SidebarProps) => {
    ...
    return (
        <div
            className={`fixed top-0 left-0 bottom-0 flex flex-col w-75 py-6 px-8 bg-background dark:bg-n-2 border-r border-n-6 overflow-auto scroll-smooth xl:z-30 md:hidden ${visible ? "w-75" : "xl:w-20"
                }`}
        >
            ...
            {isAuthenticated && (
                ...
                >
                    <div
                        className={`inline-flex items-center font-semibold text-n-9 text-sm transition-colors ${visible ? "mx-0 text-sm" : "xl:mx-auto xl:text-0"
                            }`}
                    >
                        <div
                            className={`relative w-5.5 h-5.5 mr-2.5  border border-n-6 rounded-full overflow-hidden ${visible ? "mr-2.5" : "xl:mr-0"
                                }`}
                        >
                            <Image
